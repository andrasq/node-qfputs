/**
 * Fputs -- buffered line-at-a-time output
 *
...
module.exports = (function() {
    ...
    function Fputs( writable, opts ) {
        ...
        this.writesize = opts.writesize || 204800;
        this.datachunks = [];
        this.unwrittenLength = 0;
        this._syncing = false;

        this._error = null;
        this.reportError = function(err) { this._error = err; }
    }

    /**
     * Append a line to the fifo, making sure to newline terminate it.
     */
     ...
     * Waiting wait is optional, writes are buffered anyway.
     */
    Fputs.prototype.drain = function drain( maxUnwritten, callback ) {
        ...
        if (this.unwrittenLength <= maxUnwritten) {
            callback();
        }
        else {
            var self = this;
    ...
    /**
     * Wait until all data flush activity has ceased.
     */
    Fputs.prototype.fflush = function fflush( callback ) {
        if (this._error) return callback(this._error);

        if (this._syncing || this.datachunks.length > 0) {
            var self = this;
            setTimeout(function(){ self.fflush(callback); }, 2);
        }
        else {
            callback();
        }
    ...
    Fputs.prototype._sync = function _sync( ) {
        ...
        chunk = this.datachunks.shift();

        // writing must be sequential, else node will lose data.
        ...
        this.writable.write(chunk, function(err, ret) {
            ...
            if (self.datachunks.length > 2) setImmediate(function(){ self._sync(); });
            else if (self.datachunks.length > 0) setTimeout(function(){ self._sync(); }, 5);
            else self._syncing = false;
        });
    }

    // sync is an alias for fflush, for qlogger
    // TODO: make qlogger use fflush, then deprecate this
