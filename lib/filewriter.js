...
module.exports = function FileWriter( filename, openmode ) {
    ...
    // do not overwrite contents when reopening file, use r+ instead of w
    this.reopenmode = openmode[0] === 'w' ? 'r+' : openmode;
    ...
    this._getFd = function(callback) {
        ...
        fse.open(this.filename, mode, function(err, fd) {
            ...
            // Note: this seek has race condition if not using 'a' append mode
            // this can be finessed with flock, but probably no need
            // (non-'a' access is presumably not shared)
            fse.seekSync(fd, 0, fse.SEEK_END);
            self.fd = fd;
